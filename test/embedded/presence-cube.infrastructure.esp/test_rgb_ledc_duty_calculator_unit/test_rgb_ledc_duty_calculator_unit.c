    ...
    uint32_t expected)
{
    TEST_PRINTF(
        ...
        expected);
    struct ledc_led_t led = _build_ledc_led(led_type, resolution_in_bit, percent);
...
void test_calculate_duty_percent__given_uninitialized_input__throws()
{
    struct ledc_led_t uninitialized_led = {};
    TEST_ASSERT_THROWS(ERROR_CODE_UNINITIALIZED_DATA_ACCESS, {
        ledc_led_duty_calculator__calculate_brightness_percent(&uninitialized_led);
    });
}
...
void test_calculate_duty_percent__given_common_cathode_led__returns_percentage()
{
    ...
    // max_duty = 2^4 = 16, duty = floor(16 * 50 / 100) = 8
    const uint32_t expected_duty = 8;
    struct ledc_led_t led = _build_ledc_led(LED_TYPE__COMMON_CATHODE, bit_resolution, expected_duty);

    float actual_duty_percent = ledc_led_duty_calculator__calculate_brightness_percent(&led);

    TEST_ASSERT_EQUAL(brightness_percent, actual_duty_percent);
}
...
void test_calculate_duty_percent__given_common_anode_led__returns_percentage()
{
    ...
    // max_duty = 2^4 = 16, duty_cathode = floor(16 * 25 / 100) = 4, duty_anode = 16 - 4 = 12
    const uint32_t expected_duty_anode = 12;

    struct ledc_led_t led = _build_ledc_led(LED_TYPE__COMMON_ANODE, bit_resolution, expected_duty_anode);
    TEST_ASSERT_EQUAL(expected_duty_anode, led.channel.duty);
